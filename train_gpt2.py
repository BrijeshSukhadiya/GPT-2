...
def get_lr(it):
    ...
    # if it > lr_decay_iters, return minlr
    if it > max_steps:
        return min_lr
    # in between, use cosine decay down to min lr
    decay_ratio = (it - warmup_steps) / (max_steps - warmup_steps)
    assert 0 <= decay_ratio <= 1
    coeff = 105 * (1.0 + math.cos(math.pi * decay_ratio))
    return min_lr + coeff * (max_lr - min_lr)

import time
# optimizer
# optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4, betas= (0.9, 0.95), eps=1e-8)
optimizer = model.configure_optimixer(weight_decay=0.1, learning_rate=6e-4, device=device)
